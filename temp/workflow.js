...
// Variabile di stato globale per il drag delle card
let isCardBeingDragged = false;

// Variabile per la dimensione dello stroke corrente
let currentDrawingStrokeWidth = 2; // Default a 2
...
function initializeWorkflow() {
  console.log('[LOG] Initializing workflow editor...');

  // Listener globale in fase di cattura per bloccare il mousedown durante il drag di una card
  console.log('[LOG] Adding capturing mousedown listener to document.');
  document.addEventListener('mousedown', (e) => {
    console.log(`[LOG CAPTURE MOUSE DOWN] Event target: ${e.target.className}, isCardBeingDragged: ${isCardBeingDragged}`);
    if (isCardBeingDragged) {
      e.stopImmediatePropagation();
      console.log('[LOG CAPTURE MOUSE DOWN] stopImmediatePropagation() CALLED because isCardBeingDragged is true.');
    }
  }, true); 
  
  const sidebar = document.getElementById('workflowFileSidebar');
  
  // Assicurati che la cartella Synapse Workflows esista e poi popola la sidebar
  if (window.api && window.api.ensureSynapseWorkflowsDir) {
    window.api.ensureSynapseWorkflowsDir().then(workflowsPath => {
      if (workflowsPath) {
        console.log('Workflow directory ensured at:', workflowsPath);
        populateWorkflowSidebar(workflowsPath); // Chiama la funzione per popolare la sidebar
      } else {
        console.error('Could not ensure Synapse Workflows directory.');
        if (sidebar) {
          sidebar.innerHTML = ''; // Pulisci prima
          const errMsg = document.createElement('p');
          errMsg.textContent = 'Errore nel caricamento dei workflow.';
          errMsg.style.color = 'red';
          sidebar.appendChild(errMsg);
        }
      }
    }).catch(error => {
      console.error('Error calling ensureSynapseWorkflowsDir:', error);
      if (sidebar) {
        sidebar.innerHTML = ''; // Pulisci prima
        const errMsg = document.createElement('p');
        errMsg.textContent = "Errore nell'accesso alla cartella workflow."; // Usare virgolette doppie per evitare problemi con l'apostrofo
        errMsg.style.color = 'red';
        sidebar.appendChild(errMsg);
      }
    });
  } else {
    console.warn('window.api.ensureSynapseWorkflowsDir is not available. Skipping directory creation and listing.');
    if (sidebar) {
      sidebar.innerHTML = ''; // Pulisci prima
      const warnMsg = document.createElement('p');
      warnMsg.textContent = 'API non disponibile per caricare i workflow.';
      warnMsg.style.color = 'orange';
      sidebar.appendChild(warnMsg);
    }
  }
  ...
  if (!workflowContainer) {
    ...
    // NUOVA SIDEBAR PER I FILE DEL WORKFLOW
    const workflowFileSidebar = document.createElement('div');
    workflowFileSidebar.className = 'workflow-file-sidebar'; // Nuova classe CSS per lo stile
    workflowFileSidebar.id = 'workflowFileSidebar';
    mainLayout.appendChild(workflowFileSidebar);
    // Aggiungeremo qui la logica per popolare la sidebar con i file .syn

    // Rimuoviamo la logica precedente che cercava di usare leftSidebarContainer e appSidebar per questo scopo
    ...
    // Crea la NUOVA toolbar ORIZZONTALE
    const workflowToolbar = document.createElement('div');
    workflowToolbar.className = 'workflow-horizontal-toolbar'; // Nuova classe CSS
    // La inseriamo come primo figlio di centerContainer, quindi sopra il workspace
    centerContainer.appendChild(workflowToolbar); 
    
    // Aggiungi i pulsanti alla toolbar orizzontale
    // Riorganizziamo l'HTML per permettere l'espansione
    workflowToolbar.innerHTML = `
      <div class="toolbar-main-buttons">
        ...
        <div class="toolbar-separator"></div>
        <button class="toolbar-btn dropdown-toggle-btn" title="Altre opzioni"><i class="fas fa-ellipsis-h"></i></button> <!-- Icona cambiata per chiarezza -->
      </div>
      <div class="toolbar-expandable-options">
        <button class="toolbar-btn add-kanban-btn" data-action="add-kanban"><i class="fas fa-table"></i> Tabella Kanban</button>
        <!-- Altri pulsanti espandibili qui -->
      </div>
      <div class="pencil-options-toolbar"> <!-- Nuova toolbar per la matita -->
        <button class="toolbar-btn color-btn active" title="Colore Rosso" data-color="#FF0000"><i class="fas fa-circle" style="color: red;"></i></button>
        <button class="toolbar-btn color-btn" title="Colore Blu" data-color="#0000FF"><i class="fas fa-circle" style="color: blue;"></i></button>
        <button class="toolbar-btn color-btn" title="Colore Verde" data-color="#00FF00"><i class="fas fa-circle" style="color: green;"></i></button>
        <button class="toolbar-btn color-btn" title="Colore Nero" data-color="#000000"><i class="fas fa-circle" style="color: black;"></i></button>
        <div class="toolbar-separator"></div>
        <button class="toolbar-btn stroke-decrease-btn" title="Diminuisci Stroke"><i class="fas fa-minus"></i></button>
        <input type="range" class="stroke-slider" min="1" max="50" value="2" title="Dimensione Stroke">
        <span class="stroke-value-display">2px</span>
        <button class="toolbar-btn stroke-increase-btn" title="Aumenta Stroke"><i class="fas fa-plus"></i></button>
      </div>
    `;
    ...
    centerContainer.appendChild(workspaceArea); // Aggiunto DOPO la toolbar orizzontale
    ...
    // Crea il canvas dedicato per il disegno
    drawingCanvas = document.createElement('canvas');
    drawingCanvas.id = 'drawingCanvas';
    drawingCanvas.width = 10000; // Stessa dimensione del workspaceContent
    ...
    // Imposta l'event listener per il pulsante add-note-btn nella NUOVA toolbar orizzontale
    const addNoteBtn = workflowToolbar.querySelector('.add-note-btn');
    if (addNoteBtn) {
    addNoteBtn.addEventListener('click', createNewNote);
    }

    // Gestione dell'espansione della toolbar
    const dropdownToggleBtn = workflowToolbar.querySelector('.dropdown-toggle-btn');
    const expandableOptions = workflowToolbar.querySelector('.toolbar-expandable-options');

    if (dropdownToggleBtn && expandableOptions) {
      dropdownToggleBtn.addEventListener('click', (event) => {
        event.stopPropagation();
        // workflowToolbar.classList.toggle('expanded'); // Rimosso per evitare conflitti con la toolbar matita
        expandableOptions.classList.toggle('visible');
        // Nascondi la toolbar della matita se le opzioni generali sono aperte
        const pencilToolbar = workflowToolbar.querySelector('.pencil-options-toolbar');
        if (pencilToolbar) {
          pencilToolbar.classList.remove('visible');
        }
      });

      // Gestione click sui pulsanti nelle opzioni espandibili
      expandableOptions.querySelectorAll('.toolbar-btn[data-action]').forEach(item => {
        item.addEventListener('click', (event) => {
          const action = event.target.closest('.toolbar-btn').dataset.action; // Assicurati di prendere l'azione dal pulsante
          console.log('Azione espandibile selezionata:', action);
          if (action === 'add-kanban') {
            console.log('Aggiungi Tabella Kanban (dalla toolbar espansa)');
            handleCreateKanbanBoard(); // Chiamiamo la nuova funzione
          }
          // Chiudi l'espansione dopo l'azione (opzionale, dipende dal design)
          // workflowToolbar.classList.remove('expanded');
          // expandableOptions.classList.remove('visible');
        });
      });
    }

    // Gestione della toolbar della matita
    const pencilToolBtn = workflowToolbar.querySelector('.pencil-tool-btn');
    const pencilOptionsToolbar = workflowToolbar.querySelector('.pencil-options-toolbar');

    if (pencilToolBtn && pencilOptionsToolbar) {
      pencilToolBtn.addEventListener('click', (event) => {
        event.stopPropagation();
        isPencilActive = !isPencilActive; // Toggle stato matita

        if (isPencilActive) {
          pencilToolBtn.classList.add('active'); // Rendi il pulsante visivamente attivo
          pencilOptionsToolbar.classList.add('visible');
          drawingCanvas.style.pointerEvents = 'auto'; // Abilita eventi sul drawing canvas
          workspaceArea.style.cursor = 'crosshair'; // Cambia cursore per disegnare
          // Nascondi altre toolbar se aperte
          const generalExpandableOptions = workflowToolbar.querySelector('.toolbar-expandable-options');
          ...
          drawingCanvas.style.pointerEvents = 'none'; // Disabilita eventi
          workspaceArea.style.cursor = 'grab'; // Ripristina cursore
          isDrawing = false; // Interrompi il disegno se in corso
        }
      });

      // Impedire che i click all'interno della pencil-options-toolbar la chiudano
      pencilOptionsToolbar.addEventListener('click', (event) => {
        event.stopPropagation();
      });

      // Aggiungi event listener ai pulsanti colore nella pencilOptionsToolbar
      const colorButtons = pencilOptionsToolbar.querySelectorAll('.color-btn');
      colorButtons.forEach(button => {
        button.addEventListener('click', (event) => {
          event.stopPropagation(); // Previene la chiusura della toolbar
          const selectedColor = button.dataset.color;
          currentDrawingColor = selectedColor;
          console.log('Colore di disegno selezionato:', currentDrawingColor);
      ...
      // Aggiungi event listener ai pulsanti dimensione stroke nella pencilOptionsToolbar
      const strokeButtons = pencilOptionsToolbar.querySelectorAll('.stroke-btn');
      strokeButtons.forEach(button => {
        button.addEventListener('click', (event) => {
          event.stopPropagation(); // Previene la chiusura della toolbar
          const selectedStroke = parseInt(button.dataset.stroke, 10);
          currentDrawingStrokeWidth = selectedStroke;
          console.log('Dimensione stroke selezionata:', currentDrawingStrokeWidth);

          // Rimuovi la classe 'active' da tutti i pulsanti stroke
      ...
      // Aggiungi event listener ai pulsanti dimensione stroke e slider nella pencilOptionsToolbar
      const strokeDecreaseBtn = pencilOptionsToolbar.querySelector('.stroke-decrease-btn');
      const strokeIncreaseBtn = pencilOptionsToolbar.querySelector('.stroke-increase-btn');
      const strokeSlider = pencilOptionsToolbar.querySelector('.stroke-slider');
      const strokeValueDisplay = pencilOptionsToolbar.querySelector('.stroke-value-display');

      function updateStrokeControls(newValue) {
        currentDrawingStrokeWidth = Math.max(1, Math.min(50, newValue));
        strokeSlider.value = currentDrawingStrokeWidth;
        strokeValueDisplay.textContent = `${currentDrawingStrokeWidth}px`;
        console.log('Dimensione stroke aggiornata:', currentDrawingStrokeWidth);
      }

      if (strokeSlider && strokeValueDisplay) {
        ...
        strokeSlider.addEventListener('input', (event) => {
          event.stopPropagation();
          updateStrokeControls(parseInt(event.target.value, 10));
        });
      }

      if (strokeDecreaseBtn) {
        strokeDecreaseBtn.addEventListener('click', (event) => {
          event.stopPropagation();
          updateStrokeControls(currentDrawingStrokeWidth - 1);
        });
      }

      if (strokeIncreaseBtn) {
        strokeIncreaseBtn.addEventListener('click', (event) => {
          event.stopPropagation();
          updateStrokeControls(currentDrawingStrokeWidth + 1);
        });
      }
    ...
    // Imposta gli event listeners per i pulsanti di zoom nella NUOVA toolbar orizzontale
    const zoomInBtn = workflowToolbar.querySelector('.zoom-in-btn');
    const zoomOutBtn = workflowToolbar.querySelector('.zoom-out-btn');
    const resetViewBtn = workflowToolbar.querySelector('.reset-view-btn');
...
// NUOVA FUNZIONE per popolare la sidebar dei file workflow
async function populateWorkflowSidebar(workflowsPath) {
  const sidebar = document.getElementById('workflowFileSidebar');
  if (!sidebar) {
    console.error('Workflow file sidebar element not found.');
    return;
  }

  sidebar.innerHTML = '<h3>Workflows Salvati</h3>'; // Titolo per la sidebar
  const ul = document.createElement('ul');

  if (window.api && window.api.listSynFiles) {
    try {
      const synFiles = await window.api.listSynFiles();

      if (synFiles && synFiles.length > 0) {
        synFiles.forEach(fileName => {
          const li = document.createElement('li');
          li.textContent = fileName.replace(/\.syn$/, ''); // Mostra il nome senza estensione .syn
          li.setAttribute('data-filename', fileName);
          li.addEventListener('click', () => {
            console.log(`Caricamento workflow: ${fileName}`);
            // Qui andrà la logica per caricare il workflow selezionato
            // Esempio: if (window.workflowFunctions && window.workflowFunctions.loadWorkflowFromFile) { 
            //   window.workflowFunctions.loadWorkflowFromFile(path.join(workflowsPath, fileName)); 
            // }
          });
          ul.appendChild(li);
        });
        ...
        p.textContent = 'Nessun workflow salvato.';
        ul.appendChild(p);
      }
      sidebar.appendChild(ul);
    } catch (error) {
      console.error('Error listing .syn files:', error);
      const errMsg = document.createElement('p');
      errMsg.textContent = 'Errore nel caricare elenco workflow.';
      errMsg.style.color = 'red';
      sidebar.appendChild(errMsg);
    }
  } else {
    console.warn('window.api.listSynFiles is not available.');
    const warnMsg = document.createElement('p');
    warnMsg.textContent = 'API non disponibile per elencare i file.';
    warnMsg.style.color = 'orange';
    sidebar.appendChild(warnMsg);
  }
}
...
function setupWorkflowInteractions(workspace) {
  ...
  workspace.addEventListener('mousedown', function(e) {
    // AGGIUNTO CONTROLLO PER MATITA ATTIVA
    if (isPencilActive) {
      // Se la matita è attiva, il mousedown sul workspace non deve avviare il panning.
      // Gli eventi di disegno sono gestiti dal drawingCanvas.
      return;
    }
  ...
  document.addEventListener('keydown', function(e) {
    ...
    if (e.code === 'Space' && !isSpacePressed) {
      // AGGIUNTO CONTROLLO PRELIMINARE PER MATITA ATTIVA PRIMA DI IMPOSTARE IL CURSORE
      if (isPencilActive) {
        return; // Non attivare il panning con spazio se la matita è attiva
      }
      isSpacePressed = true;
    ...
    if (e.key === 'Alt') {
      e.preventDefault();
      // AGGIUNTO CONTROLLO PRELIMINARE PER MATITA ATTIVA PRIMA DI MODIFICARE IL CURSORE
      if (isPencilActive) {
        return; // Non attivare il panning con Alt se la matita è attiva
      }
      workspace.classList.add('panning-ready');
...
function handleCanvasMouseDown(e) {
  console.log(`[LOG CANVAS MOUSE DOWN] START. Target: ${e.target.tagName}#${e.target.id}.${e.target.className}. isCardBeingDragged: ${isCardBeingDragged}`);
  
  if (isCardBeingDragged) {
    console.log('[LOG CANVAS MOUSE DOWN] Exiting: isCardBeingDragged is true.');
    return;
  }

  if (e.button !== 0) {
    console.log('[LOG CANVAS MOUSE DOWN] Exiting: Not left button.');
    return;
  }

  const closestInteractiveElement = e.target.closest('.workspace-note, .note-block, .toolbar-btn, .connection-port, .workflow-minimap, .workflow-minimap-viewport, .note-header, .note-content, .kanban-board, .kanban-column, .kanban-card, .kanban-add-column-btn, .kanban-add-card-btn, .kanban-editable-input');
  if (closestInteractiveElement) {
    console.log(`[LOG CANVAS MOUSE DOWN] Exiting: Click on interactive element: ${closestInteractiveElement.className}`);
    return;
  }
  
  console.log('[LOG CANVAS MOUSE DOWN] Proceeding to start panning...');
  e.preventDefault();
  e.stopPropagation();
...
function handleWorkspaceClick(e) {
  // AGGIUNTO CONTROLLO: Se la matita è attiva e il click proviene dal drawingCanvas (o un suo figlio diretto se ce ne fossero),
  // oppure se il target è il drawingCanvas stesso, non fare nulla.
  if (isPencilActive && (e.target === drawingCanvas || (drawingCanvas && drawingCanvas.contains(e.target)))) {
    return;
  }
...
function createConnectionFloatingBar() {
  ...
  deleteButton.innerHTML = '<i class="fas fa-trash-alt"></i>'; // Icona cestino
...
// Esponi la funzione updatePositionIndicator globalmente
window.updatePositionIndicator = updatePositionIndicator;

// Variabile per contatore Kanban Board
let kanbanBoardCounter = 0;

// Funzione per creare una nuova Kanban Board
function handleCreateKanbanBoard() {
  console.log('Creazione di una nuova Kanban Board');

  const workspaceContent = document.getElementById('workflowContent');
  if (!workspaceContent) {
    console.error('Contenitore del workspace (workflowContent) non trovato per Kanban Board');
    return null;
  }

  kanbanBoardCounter++;
  const boardId = `kanban-board-${Date.now()}-${kanbanBoardCounter}`;

  const kanbanBoard = document.createElement('div');
  kanbanBoard.className = 'kanban-board';
  kanbanBoard.id = boardId;

  // Calcola posizione centrale nel viewport attuale
  const workspace = document.getElementById('workflowWorkspace');
  let x = 200; // Default x
  let y = 200; // Default y
  if (workspace) {
    const viewportWidth = workspace.clientWidth;
    const viewportHeight = workspace.clientHeight;
    x = Math.round((-canvasOffsetX + viewportWidth / 2) / canvasScale) - 200; // Spostato un po' a sinistra per non sovrapporsi al centro esatto
    y = Math.round((-canvasOffsetY + viewportHeight / 2) / canvasScale) - 150;
  }

  kanbanBoard.style.transform = `translate(${x}px, ${y}px)`;
  kanbanBoard.style.width = 'auto'; // Larghezza automatica basata sulle colonne
  kanbanBoard.style.minWidth = '450px'; // Aumentata larghezza minima per la board
  kanbanBoard.style.minHeight = '350px'; // Aumentata altezza minima di default
  kanbanBoard.style.maxWidth = 'calc(100% - 40px)'; // Max larghezza nel viewport

  // Header della Kanban Board
  const boardHeader = document.createElement('div');
  boardHeader.className = 'kanban-board-header';
  
  const boardTitle = document.createElement('span');
  boardTitle.className = 'kanban-board-title-text';
  boardTitle.textContent = `Kanban Board ${kanbanBoardCounter}`;
  makeEditable(boardTitle, 'kanban-board-title-input', (newTitle) => {
    console.log(`Titolo board ${boardId} aggiornato a: ${newTitle}`);
    // Qui si potrebbe aggiornare un oggetto stato della board
  });
  boardHeader.appendChild(boardTitle);

  // Container per i pulsanti d'azione della board (layout orizzontale)
  const boardActionsContainer = document.createElement('div');
  boardActionsContainer.className = 'kanban-board-actions'; // Per stilizzare il layout

  const addColumnBtn = document.createElement('button');
  addColumnBtn.className = 'kanban-add-column-btn';
  addColumnBtn.innerHTML = '<i class="fas fa-plus"></i> Aggiungi Colonna';
  addColumnBtn.title = 'Aggiungi una nuova colonna alla board';
  addColumnBtn.addEventListener('click', () => {
    createKanbanColumn(kanbanBoard);
  });
  boardActionsContainer.appendChild(addColumnBtn);

  // Pulsante per eliminare l'intera board
  const deleteBoardBtn = document.createElement('button');
  deleteBoardBtn.className = 'kanban-board-delete-btn';
  deleteBoardBtn.innerHTML = '<i class="fas fa-trash-alt"></i>';
  deleteBoardBtn.title = 'Elimina questa Kanban Board';
  deleteBoardBtn.addEventListener('click', (e) => {
    e.stopPropagation();
    if (kanbanBoard.parentNode) {
      kanbanBoard.parentNode.removeChild(kanbanBoard);
      console.log(`Kanban Board ${boardId} eliminata.`);
      // TODO: Aggiornare lo stato/modello dati se la board è salvata
    }
  });
  boardActionsContainer.appendChild(deleteBoardBtn);

  boardHeader.appendChild(boardActionsContainer); // Aggiunge il container dei pulsanti
  ...
  workspaceContent.appendChild(kanbanBoard);

  // Logica di trascinamento della board (esistente)
  boardHeader.addEventListener('mousedown', (e) => {
    if (e.target.classList.contains('kanban-add-column-btn') || e.target.closest('.kanban-add-column-btn')) {
      return; // Non iniziare il drag se si clicca il pulsante aggiungi colonna
    }
    if (e.button !== 0) return;
    e.preventDefault();
    e.stopPropagation();

    let draggedBoard = kanbanBoard;
    const rect = draggedBoard.getBoundingClientRect();
    let dragOffsetXBoard = e.clientX - rect.left;
    let dragOffsetYBoard = e.clientY - rect.top;

    draggedBoard.classList.add('dragging');
    draggedBoard.style.zIndex = '1001'; // Sopra le note

    function handleBoardMouseMove(moveEvent) {
      if (!draggedBoard) return;
      moveEvent.preventDefault();

      const workspaceRect = workspace.getBoundingClientRect();
      const mouseXRelativeToWorkspace = moveEvent.clientX - workspaceRect.left;
      const mouseYRelativeToWorkspace = moveEvent.clientY - workspaceRect.top;
      const mouseXInCanvas = (mouseXRelativeToWorkspace - canvasOffsetX) / canvasScale;
      const mouseYInCanvas = (mouseYRelativeToWorkspace - canvasOffsetY) / canvasScale;
      const boardX = mouseXInCanvas - dragOffsetXBoard / canvasScale;
      const boardY = mouseYInCanvas - dragOffsetYBoard / canvasScale;

      draggedBoard.style.transform = `translate(${boardX}px, ${boardY}px)`;
      // Qui non aggiorniamo connessioni perché le board kanban potrebbero non averle direttamente
    }

    function handleBoardMouseUp() {
      if (!draggedBoard) return;
      draggedBoard.classList.remove('dragging');
      draggedBoard.style.zIndex = '';
      // Salvare lo stato della board (da implementare)
      console.log(`Kanban board ${draggedBoard.id} moved to ${draggedBoard.style.transform}`);
      draggedBoard = null;
  ...
  console.log(`Kanban Board creata con ID: ${boardId} in posizione x:${x}, y:${y}`);
  return kanbanBoard;
}

// Variabile per contatore Colonne Kanban
let kanbanColumnCounter = 0;

// Funzione per creare una nuova Colonna Kanban
function createKanbanColumn(boardElement, title = "Nuova Colonna") {
  if (!boardElement) {
    console.error("Elemento board non fornito per createKanbanColumn");
    return null;
  }
  const columnsContainer = boardElement.querySelector('.kanban-columns-container');
  if (!columnsContainer) {
    console.error("Contenitore colonne non trovato nella board:", boardElement.id);
    return null;
  }

  kanbanColumnCounter++;
  const columnId = `kanban-column-${Date.now()}-${kanbanColumnCounter}`;
  ...
  columnHeader.className = 'kanban-column-header'; // Nuova classe per l'header della colonna

  const columnTitleText = document.createElement('span');
  columnTitleText.className = 'kanban-column-title-text';
  columnTitleText.textContent = title;
  makeEditable(columnTitleText, 'kanban-column-title-input', (newTitle) => {
    console.log(`Titolo colonna ${columnId} aggiornato a: ${newTitle}`);
    // Qui si potrebbe aggiornare un oggetto stato della colonna
  });
  columnHeader.appendChild(columnTitleText);

  const addCardBtn = document.createElement('button');
  addCardBtn.className = 'kanban-add-card-btn';
  addCardBtn.innerHTML = '<i class="fas fa-plus"></i>';
  addCardBtn.title = 'Aggiungi una nuova card a questa colonna';
  addCardBtn.addEventListener('click', (e) => {
    e.stopPropagation(); // Evita che il click si propaghi ad altri handler (es. drag colonna)
    createKanbanCard(columnElement);
  });
  columnHeader.appendChild(addCardBtn);

  // Pulsante per eliminare la colonna
  const deleteColumnBtn = document.createElement('button');
  deleteColumnBtn.className = 'kanban-column-delete-btn';
  deleteColumnBtn.innerHTML = '<i class="fas fa-trash"></i>'; // Icona di cestino
  deleteColumnBtn.title = 'Elimina questa colonna';
  deleteColumnBtn.addEventListener('click', (e) => {
    e.stopPropagation();
    if (columnElement.parentNode) {
      columnElement.parentNode.removeChild(columnElement);
      console.log(`Colonna ${columnId} eliminata.`);
      // TODO: Aggiornare lo stato/modello dati
    }
  });
  ...
  // Event listener per D&D sulle colonne (cardsContainer)
  cardsContainer.addEventListener('dragover', (e) => {
    e.preventDefault(); // Necessario per permettere il drop
    e.dataTransfer.dropEffect = 'move';
    cardsContainer.classList.add('drag-over-active');
  });
  ...
  cardsContainer.addEventListener('dragleave', (e) => {
    // Controlla se il mouse sta lasciando l'elemento stesso o un suo figlio
    // Questo evita sfarfallio se ci sono elementi figli nella drop zone
    if (e.currentTarget.contains(e.relatedTarget)) return;
    cardsContainer.classList.remove('drag-over-active');
  });

  cardsContainer.addEventListener('drop', (e) => {
    e.preventDefault();
    e.stopPropagation(); // Importante per non far scattare altri drop listener (es. workspace)
    cardsContainer.classList.remove('drag-over-active');

    const cardId = e.dataTransfer.getData('text/plain');
    const draggedCardElement = document.getElementById(cardId);

    if (draggedCardElement && draggedCardElement.parentNode !== cardsContainer) {
      // TODO: Implementare logica per inserire la card nella posizione corretta (non solo append)
      // Per ora, la appende semplicemente alla fine della colonna.
      // Potremmo determinare la card più vicina al punto di drop e inserirla prima/dopo.
      cardsContainer.appendChild(draggedCardElement);
      console.log(`Card ${cardId} droppata nella colonna ${columnElement.id}`);
      // Qui si dovrebbe aggiornare lo stato (es. l'array di card della colonna nel modello dati)
    }
  });
  ...
  console.log(`Colonna Kanban creata con ID: ${columnId} nella board ${boardElement.id}`);
  return columnElement;
}

// Variabile per contatore Card Kanban
let kanbanCardCounter = 0;

// Funzione per creare una nuova Card Kanban
function createKanbanCard(columnElement, content = "Nuova Card") {
  if (!columnElement) {
    console.error("Elemento colonna non fornito per createKanbanCard");
    return null;
  }
  const cardsContainer = columnElement.querySelector('.kanban-column-cards');
  if (!cardsContainer) {
    console.error("Contenitore card non trovato nella colonna:", columnElement.id);
    return null;
  }

  kanbanCardCounter++;
  const cardId = `kanban-card-${Date.now()}-${kanbanCardCounter}`;

  const cardElement = document.createElement('div');
  cardElement.className = 'kanban-card';
  cardElement.id = cardId;
  console.log(`[LOG CREATE CARD] Card element ${cardId} creato.`);

  // Checkbox per la card
  const cardCheckbox = document.createElement('input');
  cardCheckbox.type = 'checkbox';
  cardCheckbox.className = 'kanban-card-checkbox';
  cardCheckbox.addEventListener('change', (e) => {
    e.stopPropagation(); // Evita che il click sulla checkbox si propaghi ad altri handler
    if (cardCheckbox.checked) {
      cardElement.classList.add('completed');
    } else {
      cardElement.classList.remove('completed');
    }
    // TODO: Salvare lo stato della checkbox nel modello dati della card
    console.log(`Checkbox per card ${cardId} impostata a: ${cardCheckbox.checked}`);
  });
  cardElement.appendChild(cardCheckbox);

  // Aggiungi un listener mousedown DIRETTO sulla card per debug
  cardElement.addEventListener('mousedown', (e) => {
    console.log(`[LOG CARD MOUSE DOWN] Card ID: ${cardElement.id}, Target: ${e.target.className}. Button: ${e.button}`);
    // Impedisci che il mousedown si propaghi al canvas, che altrimenti inizierebbe il panning.
    // L'evento dragstart gestirà l'inizio effettivo del drag.
    e.stopPropagation();
  });

  console.log(`[LOG CREATE CARD] Impostando draggable=true per ${cardId}`);
  cardElement.draggable = true; 

  console.log(`[LOG CREATE CARD] Aggiungendo listener dragstart per ${cardId}`);
  cardElement.addEventListener('dragstart', (e) => {
    console.log(`[LOG CARD DRAGSTART] Card ID: ${cardElement.id}. Target: ${e.target.className}`);
    e.stopPropagation(); 
    ...
    console.log(`[LOG CARD DRAGSTART] isCardBeingDragged SET TO TRUE. Card ID: ${cardElement.id}`);
    // document.body.classList.add('card-is-dragging'); 

    // console.log(`Dragging card ${cardElement.id}, isCardBeingDragged: ${isCardBeingDragged}`); // Vecchio log
  });

  cardElement.addEventListener('dragend', (e) => {
    console.log(`[LOG CARD DRAGEND] Card ID: ${cardElement.id}. Target: ${e.target.className}`);
    cardElement.classList.remove('dragging');
    isCardBeingDragged = false; 
    console.log(`[LOG CARD DRAGEND] isCardBeingDragged SET TO FALSE. Card ID: ${cardElement.id}`);
    // document.body.classList.remove('card-is-dragging');
    
    document.querySelectorAll('.kanban-column-cards.drag-over-active').forEach(col => {
      col.classList.remove('drag-over-active');
    });
    console.log(`Drag ended for card ${cardElement.id}`);
  });

  const cardContentText = document.createElement('div');
  cardContentText.className = 'kanban-card-content-text';
  cardContentText.textContent = content;
  makeEditable(cardContentText, 'kanban-card-content-input', (newContent) => {
    console.log(`Contenuto card ${cardId} aggiornato a: ${newContent}`);
    // Qui si potrebbe aggiornare un oggetto stato della card
  });
  // cardElement.appendChild(cardContentText); // Il contenuto ora va in un wrapper

  // Wrapper per contenuto e testo (per allineare con checkbox)
  const cardContentWrapper = document.createElement('div');
  cardContentWrapper.className = 'kanban-card-content-wrapper';
  cardContentWrapper.appendChild(cardContentText);
  cardElement.appendChild(cardContentWrapper);

  // Pulsante per eliminare la card
  const deleteCardBtn = document.createElement('button');
  deleteCardBtn.className = 'kanban-card-delete-btn';
  deleteCardBtn.innerHTML = '<i class="fas fa-trash-alt"></i>'; // Icona cestino
  deleteCardBtn.title = 'Elimina questa card';
  deleteCardBtn.addEventListener('click', (e) => {
    e.stopPropagation();
    if (cardElement.parentNode) {
      cardElement.parentNode.removeChild(cardElement);
      console.log(`Card ${cardId} eliminata.`);
      // TODO: Aggiornare lo stato/modello dati
    }
  });
  cardElement.appendChild(deleteCardBtn);

  // Eventuale header/azioni per la card qui...

  cardsContainer.appendChild(cardElement);
  console.log(`Card Kanban creata con ID: ${cardId} nella colonna ${columnElement.id}`);
  return cardElement;
}

// Funzione helper per rendere un elemento di testo editabile
function makeEditable(textElement, inputClassName = 'kanban-editable-input', onSaveCallback) {
  textElement.addEventListener('dblclick', (e) => {
    console.log(`[LOG MAKE EDITABLE DBLCLICK] Element: ${textElement.className}, Target: ${e.target.className}`);
    e.stopPropagation(); 
    ...
    function saveChanges(triggeredByEnter = false) {
      const newValue = input.value.trim();
      textElement.textContent = newValue === '' ? oldValue : newValue;
      textElement.style.display = ''; 
      if (input.parentNode) {
        input.parentNode.removeChild(input);
      }
      if (onSaveCallback && newValue !== '' && newValue !== oldValue) {
    ...
    const handleKeyDown = (event) => {
      if (event.key === 'Enter') {
        ...
        textElement.textContent = oldValue; 
        textElement.style.display = '';
        if (input.parentNode) {
          input.parentNode.removeChild(input);
        }
...
function handleDrawingStart(e) {
  if (!isPencilActive || e.button !== 0) return; // Solo tasto sinistro e matita attiva
  isDrawing = true;
  const coords = getMousePosOnDrawingCanvas(e);
  console.log('[DRW_START]: Mouse down on drawingCanvas. Coords:', coords, 'Color:', currentDrawingColor, 'Stroke:', currentDrawingStrokeWidth, 'Target:', e.target.id);

  // Test: disegna un piccolo rettangolo verde al punto del click
  if (drawingCtx) {
    drawingCtx.fillStyle = 'rgba(0,255,0,0.7)'; // Verde semi-trasparente
    drawingCtx.fillRect(coords.x - 3, coords.y - 3, 6, 6);
  }
  
  drawingCtx.beginPath();
  drawingCtx.moveTo(coords.x, coords.y);
  lastDrawingPoint = coords;
  e.stopPropagation(); 
  e.preventDefault(); // AGGIUNTO per prevenire azioni di default
}

function handleDrawingMove(e) {
  if (!isDrawing || !isPencilActive) return;
  const coords = getMousePosOnDrawingCanvas(e);
  console.log('[DRW_MOVE]: Drawing to', coords);
  
  drawingCtx.lineTo(coords.x, coords.y);
  drawingCtx.strokeStyle = currentDrawingColor;
  drawingCtx.lineWidth = currentDrawingStrokeWidth; 
...
function handleDrawingEnd(e) {
  if (!isDrawing || !isPencilActive) return;
  console.log('[DRW_END]: Mouse up/leave on drawingCanvas. Target:', e.target.id);
  isDrawing = false;
  e.stopPropagation();
  e.preventDefault(); // AGGIUNTO
}
